package com.suda.AS3BlipLib
{
	...
	public class BlipAPI extends URLLoaderBase
	{
		...
		public function GetUserDashboard(login:String, since:int = -1):void
		{
			...
			url += '?include=user,user[avatar],pictures';	
		...
		public function onGetSubs(event:Event):void
		{
			...
			for each (var item:Object in data) {
				var subscription:BlipSubscription = 
						blipSubscriptions.getByMany([{name:'trackingUserPath', value:item.tracking_user_path},
													 {name:'trackedUserPath', value:item.tracked_user_path}]);
				if (null == subscription) {										
					subscription = new BlipSubscription(item);
					blipSubscriptions.addItem(subscription);	
				} else {
					subscription.transports.push(item.transport);
				}
			}
